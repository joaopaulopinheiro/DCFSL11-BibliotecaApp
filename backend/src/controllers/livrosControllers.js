...
// Retorna o catálogo de livros com disponibilidade dinâmica
export async function getCatalogo(req, res) {
  try {
    ...
    // Calcular disponibilidade em tempo real para cada livro
    const catalogoComDisponibilidade = await Promise.all(
      livros.map(async (livro) => {
        // Contar pedidos ativos (pendente, aprovado) para este livro
        ...
        const disponivel = (livro.estoque || 0) - pedidosAtivos;

        return {
          ...livro,
          disponivel: Math.max(0, disponivel), // nunca retorna número negativo
          estoque_total: livro.estoque || 0,
          pedidos_ativos: pedidosAtivos
        };
      })
    );

    // Filtrar livros baseado no perfil do usuário
    const usuarioPerfil = req.usuario?.perfil; // vem do validateToken

    let resultadoFinal = catalogoComDisponibilidade;

    // Se for aluno, não mostrar livros sem disponibilidade
    if (usuarioPerfil === 'aluno') {
      resultadoFinal = catalogoComDisponibilidade.filter(livro => livro.disponivel > 0);
    }
    // Se for colab ou admin, mostrar todos com disponibilidade

    return res.json(resultadoFinal);
  } catch (err) {
    console.error('Prisma query failed (getCatalogo):', err);
    return res.status(500).json({ error: 'Database connection error' });
  }
}
...
export async function deleteLivro(req, res) {
  ...
  try {
    // Verificar se existem pedidos vinculados a este livro
    const pedidosVinculados = await prisma.pedidos.count({
      where: { livro_id: id }
    });

    if (pedidosVinculados > 0) {
      return res.status(409).json({ error: 'Não é possível deletar: existem pedidos vinculados a este livro.' });
    }
